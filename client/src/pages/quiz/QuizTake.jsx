...
import { asyncgetquizbyid, asyncsubmitquiz } from '../../store/actions/quizActions';
...
const QuizTake = () => {
  ...
  useEffect(() => {
    dispatch(asyncgetquizbyid(quizId))
      .then((data) => {
        setQuiz(data);
        setLoading(false);
      })
    ...
    return (
      <div className='flex items-center justify-center min-h-[60vh]'>
        <div className='text-center'>
          <p className='text-slate-600 text-lg'>No quiz found or quiz has no questions.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className='max-w-4xl mx-auto'>
      ...
      <div className='bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-6 mb-8'>
        {/* Question label */}
        <div className='inline-flex items-center gap-2 px-3 py-1 bg-linear-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-xl mb-4'>
        ...
        <div className='flex items-center justify-between gap-4'>
          ...
          {currentQuestionIndex === quiz.questions.length - 1 ? (
            <button
              ...
              disabled={submitting}
              className='group relative px-8 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-500 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100 overflow-hidden'
            >
